	...
	while (j < slg->y_len)
	{
		...
		while (i < slg->x_len)
		{
			if (slg->tab[j][i] == '1')
			{
				...
				real_draw(slg, "texture/back_snow.xpm", i, j);
				real_draw(slg, "texture/ice.xpm",  i, j);
			}
			if (slg->tab[j][i] == '0')
			...
			if (slg->tab[j][i] == 'C')
			{
				...
				real_draw(slg, "texture/back_snow.xpm", i, j);
				real_draw(slg, "texture/heart.xpm",  i, j);
			}
			if (slg->tab[j][i] == 'P')
			{
				// draw_image(slg, i, j, 0x009FFF00);	
				real_draw(slg, "texture/back_snow.xpm", i, j);
				real_draw(slg, slg->img.player_cur, i, j);
			}
			if (slg->tab[j][i] == 'E')
			{
				// draw_image(slg, i, j, 0x00000055);	
				real_draw(slg, "texture/back_snow.xpm", i, j);
				real_draw(slg, "texture/ice_hole.xpm",  i, j);
			}
			if (slg->tab[j][i] == 'V')
			{
				// draw_image(slg, i, j, 0x00000055);	
				real_draw(slg, "texture/back_snow.xpm", i, j);
				real_draw(slg, "texture/sea_lion.xpm",  i, j);	
			}
			i++;
		}
		j++;
